...
Ember & Vale is a custom Next.js ecommerce site for a premium large-candle brand. It uses a calm luxury storefront, Printify product fulfillment, Stripe checkout, persistent cart state, account pages, admin screens, Prisma models, Resend transactional email hooks, and Cloudinary-ready product imagery.
...
- Stripe Checkout, PaymentIntents, and webhook route
- Printify product and order API integration
- Resend email client
- Cloudinary SDK
...
3. Fill in `DATABASE_URL`, `NEXTAUTH_SECRET`, `NEXTAUTH_URL`, Stripe, Printify, Google, Resend, and Cloudinary values.
...
- `PRINTIFY_API_KEY`: Printify personal access token used to fetch products and create fulfillment orders.
...
4. Add the deployed webhook URL in Stripe: `/api/webhooks/stripe`. This webhook creates Printify orders after Stripe Checkout completes.
